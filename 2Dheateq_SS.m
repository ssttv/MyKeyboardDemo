...
T_east   = 25 ;                   % temperature on the upper side ( at y=0  "Dirichlet Conditions" )
T_west   = 40 ;                   % temperature on the lower side ( at y=Ly "Dirichlet Conditions" )
T_north  = 50 ;                   % temperature on the left  side ( at x=0  "Dirichlet Conditions" )
T_south  = 35 ;                   % temperature on the right side ( at x=Lx "Dirichlet Conditions" ) 
...
Solution_type=questdlg('Метод определения производной по времени','Question','Эйлер','Рунг-Кутт','Эйлер');                     % solve with 2nd order Runge Kutte in time or 2 to solve with Euler

if dt<= 1/(2*alpha*((1/dx^2)+(1/dy^2)))                             % test the stability condition 
else 
    fprintf('Ошибка\nВведите новые данные %f \n',1/(2*alpha*((1/dx^2)+(1/dy^2))))
    return
end
message=msgbox('Идет решение, ждите..... ');    % Busy message 
...
switch Solution_type
    ...
    while err_R_k_max(k)>=tolerence || err_R_k_min(k)>=tolerence
      ...
      if round(err_R_k_max(k),5)==round(err_R_k_max(k-1),5) && err_R_k_max(k)~= 0      % Test solution convergence
       errordlg('Решение не сходится','Tolerence Error');
       close(message)
       return
      end
    ...
    while err_E_max(k)>=tolerence || err_E_min(k)>=tolerence
      ...
      if round(err_E_min(k),5)==round(err_E_min(k-1),5) && err_E_min(k)~= 0      % Test solution convergence
       errordlg('Решеие не сходится','Tolerence Error');
       close(message)
       return
      end
     end
...
fprintf('Пластина имеет измерения %i X %i и изготовлена из материала %s \n',Lx,Ly,name);
fprintf('Начальные значения для граничных условий Дирихле \n')
fprintf('T(x,0,t)=%i , T(x,%i,t)=%i , T(0,y,t)=%i , T(%i,y,t)=%i , T(x,y,0)=%i \n',T_south,Ly,T_north,T_west,Lx,T_east,T_initial)
fprintf('Требуется %i секунд для достижения устойчивого состояния с точностью %0.2f \n',round(SStime),tolerence);
fprintf('Симуляция проводится %i секунд с шагом в %0.2f секунд \n',t_end,dt)
...
subplot(2,2,3)                           
hold on
title(sprintf('Температура в устойчивом состоянии: %i секунд ',round(SStime)))
surf(x,y,Tss)
...
caxis([T_min T_max]);
view(90,-90);
xlim([0 Lx+dx]); xlabel('Длина');
ylim([0 Ly+dy]); ylabel('Ширина');
zlim([T_min T_max]); zlabel('Температура');
drawnow
hold off
...
axis tight; xlabel('Временные итерации');
ylim([T_min T_max]); ylabel('Температура');
...
surf(x,y,T(:,:,j))
hold on
title(sprintf('Темперауура в момент : %i секунд ',round(j*dt)))
plot3(  Lx/4,  Ly/4,T_max,'ko','markerfacecolor','r') % plot red point
plot3(  Lx/2,  Ly/2,T_max,'ko','markerfacecolor','g') % plot green point
plot3(3*Lx/4,3*Ly/4,T_max,'ko','markerfacecolor','b') % plot blue point
plot3(  Lx/4,  Ly/4,T_min,'ko','markerfacecolor','r') % plot red point
...
subplot(2,2,2)
hold on
title(sprintf('Температуура в момент : %i секунд ',round(j*dt)))
